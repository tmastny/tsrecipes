...
reduction
for time series that improve accuracy in time series classification and clustering.
...
1. representing time series as combinations of different
   time series
2. explaining how the discrete cosine transform is a series of cosine waves
...
Lastly, I share some resources on data compression: the field of study
where DCT originated, and why it has some fundamental connections to
machine learning. 
...
smoothing, like the trend in classic decomposition (or a simple moving average).
It's not as useful for as other smoothing techniques, in general,
because it a little too wavy. You can see the blue waves that randomly appear in
...
## Discrete cosine transform as time series cosine waves
...
Overall, still pretty good, but the 0-500 and >1500 look a little too wiggly.
...
With the last time series, we first thought to try exclude the high-frequency
...
dimensionality. Principle Component Analysis (PCA) can
