...
1. how standard distance metrics lead failure for time series clustering
...
usually called the Mean Squared Error (MSE) or 
...
plot_dtw(eth_sample_trunc$ts[[1]], eth_sample_trunc$ts[[2]])
```
...
```{r}
plot_dtw(eth_sample_trunc$ts[[1]], eth_sample_trunc$ts[[3]])
```
